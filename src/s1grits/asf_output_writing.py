...
def _generate_preview_png(
    ...
    Output format:
    - When ratio is None: 3-band RGB [R=VV, G=VH, B=VV] (VV duplicated to blue channel)
    - When ratio is present: 3-band RGB [R=VH, G=VV, B=Ratio]

    Notes:
        When ratio is absent, VV is duplicated to the blue channel to maintain
        OpenCV compatibility (which requires 1, 3, or 4 channels, not 2).
        This results in cyan-tinted imagery where vegetation (high VH) appears greenish.
    ...
    # RGB output: stack bands based on what's available.
    if len(normalized_bands) == 2:
        # Only VV and VH: create 3-band RGB [R=VV, G=VH, B=VV]
        # Duplicate VV to blue channel for OpenCV compatibility (requires 1/3/4 channels, not 2)
        # Order in normalized_bands: [0]=VV_dB, [1]=VH_dB
        vv_band = normalized_bands[0]  # VV
        vh_band = normalized_bands[1]  # VH
        rgb = np.stack([vv_band, vh_band, vv_band], axis=-1)  # R=VV, G=VH, B=VV
    elif len(normalized_bands) == 3:
        # VV, VH, and Ratio: create 3-band RGB [R=VH, G=VV, B=Ratio]
        # Order in normalized_bands: [0]=VV_dB, [1]=VH_dB, [2]=Ratio
        rgb = np.stack([normalized_bands[1], normalized_bands[0], normalized_bands[2]], axis=-1)
    else:
...
def _flush_month_crossUTM(
    ...
    # H) Write COG
    # STAC mode: filename includes direction_label and feature_suffix
    ...
    if generate_cog and (overwrite_cog or not os.path.exists(cog_p)):
        ...
        with rasterio.open(cog_p, "w", **master_profile) as dst:
            _bi = 1
    ...
    if generate_preview:
        try:
            # STAC mode: preview filename matches COG naming
...
def _check_tile_integrity(
    ...
    """
    Check product directory integrity before processing.

    Zarr is the authoritative Data Cube product. If COG/preview exist
    but the Zarr store is missing, the product is in an inconsistent state —
    delete the product directory (NOT the entire tile) and start fresh to
    ensure temporal alignment.

    **CRITICAL FIX (2026-06-16)**: This function now checks and cleans only
    the specific product directory, not the entire tile directory. This allows
    multiple product types (monthly, scenes, static) to coexist in the same tile
    without accidentally deleting each other.
    ...
    Args:
        tile_dir: Tile directory (e.g., "46SEG/")
        zarr_path: Path to zarr store for this product
        product_label: Product type label (e.g., "monthly_DESCENDING")

    Returns:
        True if product is consistent (safe to proceed).
        False if product was inconsistent and has been cleaned up.

    Raises:
        RuntimeError if cleanup fails (e.g. Windows file locks).
    """
    ...
    # Check product-specific artifacts (COG and preview)
    cog_dir = os.path.join(product_dir, "cog")
    ...
    if not cog_exists and not preview_exists:
        return True  # Nothing exists for this product — fresh start, consistent

    # Zarr missing but COG/preview present for THIS product — inconsistent state.
    # COG/preview files without a Zarr cube are useless for Data Cube operations
    # and represent a corrupted state. Must rebuild this product from scratch.
    logging.warning(
        "[Integrity] Zarr Data Cube missing for product '%s' but stale "
        "COG/preview artifacts found in %s. This indicates a previous run "
        "that did not complete Zarr generation. Removing PRODUCT directory "
        "(not entire tile) for clean rebuild.", product_label, product_dir
    )

    # Safety check: ensure we're only deleting within tile_dir
    _expected_prefix = os.path.realpath(os.path.abspath(tile_dir))
    _actual_path = os.path.realpath(os.path.abspath(product_dir))
    if not _actual_path.startswith(_expected_prefix):
        raise ValueError(
            f"Refusing to remove path outside tile_dir: {product_dir}"
        )

    # 🎯 KEY FIX: Remove only the product directory, not the entire tile directory
    # This allows multiple products (monthly, scenes, static) to coexist safely
    _rmtree_windows_safe(product_dir)
    logging.info("[Integrity] Removed product directory: %s", product_dir)
    return False
...
def _zarr_delete_timestep(g, m_str: str) -> None:
    ...
    n_before = len(times)
    band_vars = [k for k in g.keys() if k not in ("time", "x", "y")]
...
def build_s1_monthly_cog_and_zarr_crossUTM(
    ...
    **kwargs,
    ...
    # STAC-compliant directory structure
    ...
    _check_tile_integrity(tile_dir, zarr_path, product_label)
...
def merge_acq_group_zarrs(
    ...
    _first_arr = next(arr for name, arr in master_store.arrays() if name not in ('x', 'y', 'time'))
        ...
        if generate_cog:
            ...
            if overwrite_cog or not os.path.isfile(cog_p):
                try:
                    ...
                    with rasterio.open(cog_p, "w", **merge_profile) as dst:
...
def _upsert_tile_catalog(catalog_path: str, new_records: list) -> int:
    ...
    dedup_keys = ["tile_id", "flight_direction", "month", "product_type"]
    ...
    combined.to_parquet(catalog_path, index=False)
    return len(combined)
...
def merge_tile_catalogs(output_root: str, output_catalog: str = None):
    ...
    combined.to_parquet(output_catalog, index=False)
