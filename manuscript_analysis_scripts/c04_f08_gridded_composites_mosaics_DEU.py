...
DATA_FOLDER_NAME = "component_04_multi_region_spatial_scalability_monthly_composites"
...
INPUT_VRT = DATA_BASE_DIR / "vrt" / "32TNT-33UVQ-2026-03-DESCENDING-4326.vrt"
INPUT_JSON_PARAMS = DATA_BASE_DIR / "vrt" / "vrt_stretch_params_Ecuador.json"

# Portable fallback checking paths for vector boundaries to eliminate absolute path dependency
POTENTIAL_SHP_PATHS = [
    SCRIPT_DIR / "boundary" / "germany_adm1_downloaded_nurnberg.shp",
    DATA_BASE_DIR / "boundary" / "germany_adm1_downloaded_nurnberg.shp",
    DATA_BASE_DIR / "figure" / "boundary" / "germany_adm1_downloaded_nurnberg.shp"
]
...
if INPUT_SHP is None:
    ...
    INPUT_SHP = SCRIPT_DIR / "boundary" / "germany_adm1_downloaded_nurnberg.shp"

OUTPUT_DIR_PNG = DATA_BASE_DIR / "figure_GSRS_Fig8abcd"
...
GRID_INTERVAL = 1.0  # 1.0 degree spacing optimized for regional macro-scale map overview
...
def create_s1_false_color_composite(
    ...
    with adaptive hemisphere coordinate labels and high-zorder gridlines. Legend block is disabled.
    """
    ...
    if show_grid:
        ...
        lon_labels = [f"{x:.1f}°E" if x >= 0 else f"{abs(x):.1f}°W" for x in x_ticks]
        lat_labels = [f"{y:.1f}°N" if y >= 0 else f"{abs(y):.1f}°S" for y in y_ticks]
    ...
    # =========================================================================
    # 🌟 Legend section remains cleanly disabled/commented out
    # =========================================================================
    # legend_elements = [
