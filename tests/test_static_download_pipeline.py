...
def test_downloaded_static_layer_is_written_on_reference_grid(tmp_path, monkeypatch):
    ...
    def fake_cog(_arr, _transform, _crs, out_path, *_args, **_kwargs):
        Path(out_path).parent.mkdir(parents=True, exist_ok=True)
        Path(out_path).touch()

    monkeypatch.setattr(ws, "_build_static_cog", fake_cog)
    ...
    # Zarr v3 arrays do not implement len(); an idempotent rerun must validate
    # shape through array.shape and return before any network download.
    calls.clear()
    rerun = ws._process_one_static_group(
        rows=rows,
        layers=["local_inc_angle"],
        static_dir=static_dir,
        mgrs_tile_id=tile,
